...
Audit against `~/.local/state/mise-migration/20260914-173012` and the live workstation. No build, installer, or package update was run.

## Pi package cutover

Keep these five references in the Pi user settings, not as mise `npm:` tools:
...
All five are installed in the current npm global root, but each package has no `bin` entry. Pi 0.80.3 lists them in `~/.pi/agent/settings.json`; its legacy package lookup uses `npm root -g`, currently `/opt/homebrew/lib/node_modules`. A Node owner change changes that global root, while mise installs npm tools in isolated package roots. The plugins can then disappear from Pi discovery.

Pi 0.80.3 supports a stable user install: `PI_CODING_AGENT_DIR="$HOME/.pi/agent" pi update npm:<package> --no-approve` writes packages under `~/.pi/agent/npm/node_modules`. The update checks this managed path and installs there when it is absent. `--no-approve` excludes project-local settings. The new [Pi plugin setup task](../tasks/setup/exceptions/pi-plugins) checks that the five user refs exist, updates only those refs, and checks their install directories. It runs after native tools during bootstrap and during routine updates. The pre-dotfiles hook seeds those five public package references only when the settings file is absent; existing settings are preserved and enrolled in encrypted private history. Live plugin ownership transfer remains pending.

Other npm declarations are ordinary CLIs with binaries and suit mise: `pi` (`@earendil-works/pi-coding-agent`), `codex`, `codex-acp`, and `agent-browser`. Keep `portless` project-owned; it is a symlink to its checkout. `npm` itself comes with Node.
...
| Command | Live resolution | Workstation declaration and compatibility |
| --- | --- | --- |
| `node` / `npm` | Mise shim falls through to Homebrew Node 26.7.0 at `/opt/homebrew/Cellar/node/26.7.0/bin/node`; `npm root -g` is `/opt/homebrew/lib/node_modules`. | `node = "latest"` can own the runtime. Its global npm root changes, which is why Pi's five plugins need the Pi-managed path above. No retained dotfile pins the Node 22 keg path. |
...
| `codex` | Homebrew's npm CLI 0.153.4 at `/opt/homebrew/lib/node_modules/@openai/codex/bin/codex.js`. ChatGPT.app also has its own app-owned executable. | The mise npm package has a `codex` binary, so the CLI can move to mise while the app remains separate. |

The audit snapshot had seven global uv tools: `keymap-drawer`, `maturin`, `mlx-audio`, `mlx-vlm`, `mlx-whisper`, `typ2docx`, and `worldlines`. Five remain `pipx:` declarations: `keymap-drawer`, the three MLX tools, and `worldlines`. `maturin` now uses the PyO3 GitHub release backend with `filter_bins = "maturin"`; its binary was checked for macOS arm64, Linux x64, and Linux arm64. `typ2docx` is removed from `[tools]` because the locked pipx install cannot build its source distribution. The named [typ2docx exception](../tasks/setup/exceptions/typ2docx) builds a version-pinned release with Mise Python 3.13, Rust, and Cargo in per-run UV and Cargo directories, then switches `~/.local/bin/typ2docx` only after `--help` passes. It backs up a recognized prior UV symlink, leaves the default UV and Cargo directories alone, skips a verified prior install during `--install-only`, and skips unchanged releases during routine updates. `xonsh` is the sixth pipx declaration, moving from Homebrew 0.24.2 to mise pipx; it was not part of the seven-tool UV snapshot. The MLX declarations explicitly select Python 3.13.
