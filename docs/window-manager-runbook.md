...
6. Run `rift-local-install --activate` only for the cutover. It snapshots the active binaries, current pointer, links, live config, `~/.rift/layout.ron`, LaunchAgent, receipt, and exact service state before stopping anything. With Rift stopped, it copies the signed binaries into the permanent real directory `~/.local/state/rift-local-install/active/`, using atomic file renames. Both command links point into `active`; `current` records the selected archived release only. It installs the tracked assets, starts the candidate, verifies its Mach endpoint, and writes the receipt. Any activation failure restores the complete snapshot. If rollback itself fails, the installer reports it and keeps the snapshot for manual recovery.
7. Grant Device Control and Data Access and Input Monitoring to the permanent `active/rift` executable once if macOS asks, reload the config, and inspect the live service. Keep the same signing certificate and real executable path on later updates. The first migration may need more approval time, which can be set with `RIFT_SERVICE_READY_ATTEMPTS=600 rift-local-install --activate`. Remove obsolete Rift permission entries only with Rift stopped, and keep the permanent entry. Do not use an environment flag or Chezmoi apply to activate Rift.
