...
case "${1:-} ${2:-} ${3:-}" in
    ...
    'config set --file')
        ...
        ;;
    'ls --current --json')
        printf '{"github:fish-shell/fish-shell":[{"active":true,"installed":true,"install_path":"%s"}]}\\n' "$MISE_FISH_INSTALL"
        ;;
...
""",
    ...
    def test_mise_fish_gets_a_stable_latest_link(self) -> None:
        install = self.home / ".local/share/mise/installs/github-fish-shell-fish-shell/latest"
        ...
        self.assertEqual(os.readlink(stable), str(fish))
    ...
    def test_noninteractive_without_passwordless_sudo_defers_before_config_write(self) -> None:
        fish = self.bin / "fish"
        self._write_fish(fish)
        self.env["SUDO_OK"] = "0"
