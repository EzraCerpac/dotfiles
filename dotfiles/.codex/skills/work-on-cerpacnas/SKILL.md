...
3. In the mise setup source (`~/.config/mise`), use `nas config status|update`; do not select a manifest project.
...
2. Read [project manifest](references/projects-manifest.md). Edit the manifest through its mise setup source, never the rendered file. Add a rendered project-rules file too; reuse supplied repository instructions when available.
...
4. Validate the mise configuration and skill. Commit configuration on the configured dotfiles branch. Publishing still requires explicit consent when current instructions prohibit pushing.
...
- In the mise setup source (`~/.config/mise`), use `nas config status` then `nas config update`. The update fetches the configured JJ branch, shows `dots plan`, and applies with `dots apply`; it never publishes source.
