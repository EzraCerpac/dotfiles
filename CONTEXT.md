...
The normal Chezmoi adapter installs the official Aegis 1.1.0 bundle. This machine can instead build nine stacked changes from the exact pinned revision of `~/Projects/Tools/Aegis`: modified Cmd+Tab handling, a menu-only context button, configurable workspace labels, native-fullscreen bar hiding, empty-workspace filtering, readable system-native settings, app-switcher event-tap recovery, bounded Rift subscription reconnection, and Screen Recording-aware window previews.
