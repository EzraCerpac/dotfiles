...
The normal Chezmoi adapter installs the official Aegis 1.1.0 bundle. This machine can instead build the exact pinned `local/aegis-ezra-build` revision from `~/Projects/Tools/Aegis`. That local integration combines six independently reviewable branches based on upstream `main`: `pr/rift-reliability`, `pr/switcher-permissions`, `pr/system-ui`, `pr/workspace-controls`, `pr/native-glass`, and `pr/switcher-actions`. A separate final commit suppresses Accessibility prompts and global event taps only inside XCTest hosts. The local build also retains the tested Rift subscription controller under `archive/aegis-rift-reconnect-alternative` while upstream PR #7 resolves the overlapping daemon-restart work.
