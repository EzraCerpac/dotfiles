...
The normal Chezmoi adapter installs the official Aegis 1.1.0 bundle. This machine can instead build four stacked changes from the exact pinned revision of `~/Projects/Tools/Aegis`: modified Cmd+Tab handling, a menu-only context button, configurable workspace labels, and native-fullscreen bar hiding.
