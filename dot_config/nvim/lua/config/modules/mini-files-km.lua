...
M.setup = function(opts)
  ...
  -- Map to open the highlighted directory in a right-side Vim vertical split
  -- (no tmux dependency)
  vim.api.nvim_create_autocmd("User", {
    ...
    callback = function(args)
      ...
      -- Open a right vsplit from the target window and "go in" on the entry:
      -- - If it's a file, it opens in the new split (target window).
      -- - If it's a directory, MiniFiles navigates to it (still using floats).
      vim.keymap.set("n", keymaps.open_split_right, function()
        ...
        -- Make the new split the target for file opens
        mini_files.set_target_window(new_win)
        -- Delegate behavior to MiniFiles: file opens in target split; directory navigates
        mini_files.go_in()
      end, { buffer = buf_id, noremap = true, silent = true, nowait = true, desc = "MiniFiles: vsplit and go in" })
